...
public class ExpressionTests {
    ...
    public void testNestedExpressions() {
        Assert.assertTrue((boolean) CompilerTest.run("(eq #f (> (- (+ 1 3) (* 2 (mod 7 4))) 4))"));
    }
    ...
    @Test
    public void testIf() {
        Assert.assertEquals(42, (int) CompilerTest.run("(if #t 42 1000)"));
    }

    /* The branch of the if statement that is discarded should never be evaluated. */
    @Test
    public void testIncorrectBranchIsNotExecuted() {
        Assert.assertEquals(42, (int) CompilerTest.run("(if #f (+ 42 'foo) 42)"));
    }
    ...
    public void testIfWithSubExpression() {
        Assert.assertEquals(42, (int) CompilerTest.run(" (if (> 1 2)\n"
                + "(- 1000 1)\n"
                + "(+ 40 (- 3 1)))"));
    }
